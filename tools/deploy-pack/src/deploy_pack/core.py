...
def load_project_policy(root: Path) -> tuple[list[str], list[str]]:
    ...
    if not isinstance(ignore, list) or not all(isinstance(v, str) for v in ignore):
        raise DeployPackError(f"{CONFIG_FILE}: ignore must be an array of strings")
    if not isinstance(include, list) or not all(isinstance(v, str) for v in include):
        raise DeployPackError(f"{CONFIG_FILE}: include must be an array of strings")
...
def build_plan(
    ...
    *,
    ...
    policy_ignores, policy_includes = load_project_policy(root)
    ignores = DEFAULT_IGNORE_PATTERNS + tuple(policy_ignores) + tuple(cli_ignores)
    includes = tuple(policy_includes) + tuple(cli_includes)

    def ignored(path: str) -> bool:
        if any(matches_pattern(path, p) for p in includes):
            return False
        return any(matches_pattern(path, p) for p in ignores)
    ...
    for change in changes:
        # Security boundary: normal include policy cannot resurrect deploy-pack
        # state, evidence, secret-bearing verifiers, or generated custody files.
        ...
        if "D" in change.status:
            ...
            continue
        if ignored(change.path):
            ignored_changes.append(change)
            if (
                change.status.startswith("R")
                and change.old_path
                and not ignored(change.old_path)
                and not protected(change.old_path)
            ):
...
def build_rollback_plan(
    ...
    *,
    ...
    for change in changes:
        ...
        if ignored(change.path):
            ignored_changes.append(change)
            if change.status.startswith("R") and change.old_path and not ignored(change.old_path):
